        ...
        <div className="p-8 animate-in fade-in duration-300">
            ...
            <div className="flex items-center gap-6 mb-8">
                <div className="relative">
                    ...
                    <button className={clsx("absolute bottom-0 p-1.5 bg-white rounded-full border border-gray-200 shadow-sm text-gray-500 hover:text-indigo-600 transition-colors", isAr ? "left-0" : "right-0")}>
                        <Camera className="w-4 h-4" />
                    </button>
                </div>
                <div>
                    <h4 className="text-lg font-bold text-gray-900">{user?.name || t('user')}</h4>
                    <p className="text-sm text-gray-500 capitalize">{user?.role || t('standardUser')}</p>
                    <button className="mt-2 text-xs font-bold text-gray-600 bg-gray-100 px-3 py-1.5 rounded-lg hover:bg-gray-200 transition-colors">{t('changePhoto')}</button>
                </div>
            </div>
