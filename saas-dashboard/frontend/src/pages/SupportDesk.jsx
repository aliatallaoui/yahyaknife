...
export default function SupportDesk() {
    ...
    const handleSelectTicket = async (ticket) => {
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/support/${ticket._id}`);
            const data = await res.json();
    ...
    const handleSendReply = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/support/${selectedTicket._id}/messages`, {
                method: 'POST',
    ...
    const handleUpdateStatus = async (status) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/api/support/${selectedTicket._id}/status`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
            {/* Sidebar List */}
            <div className="w-[400px] bg-white border-r border-gray-200 flex flex-col h-full z-10 shadow-sm relative">
                <div className="p-5 border-b border-gray-100 bg-white/50 backdrop-blur-md sticky top-0">
                    ...
                    <div className="relative mb-4">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none"
                        />
                    </div>

                    <div className="flex gap-2">
                        {['All', 'Open', 'Waiting on Customer', 'Resolved'].map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={clsx(
                    ...
                    {loading ? (
                        ...
                        filteredTickets.map(ticket => (
                            <button
                                ...
                                className={clsx(
                                    "w-full text-left p-4 rounded-xl transition-all duration-200 group relative mb-1",
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-bold text-gray-900 truncate pr-2">{ticket.customerId?.name || 'Unknown'}</span>
                                    </div>
                                    <span className="text-xs font-medium text-gray-400 whitespace-nowrap">
                                        {moment(ticket.createdAt).fromNow(true)}
                                    </span>
                                </div>
                                <p className="text-sm font-semibold text-gray-700 truncate pr-8 mb-2">
                                    {ticket.subject}
                                </p>
            ...
            <div className="flex-1 flex flex-col bg-white h-full relative">
                {selectedTicket ? (
                    <>
                        {/* Header Details */}
                        <div className="p-6 border-b border-gray-100 bg-white flex justify-between items-start shadow-sm z-10 shrink-0">
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    <h2 className="text-2xl font-black text-gray-900 tracking-tight">{selectedTicket.subject}</h2>
                                    <span className={clsx("px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider border", getPriorityColor(selectedTicket.priority))}>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
                            <div className="max-w-3xl mx-auto space-y-6 pb-4">
                                {selectedTicket.messages.map((msg, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className={clsx("flex gap-4 max-w-[85%]", isAgent ? "ml-auto flex-row-reverse" : "")}>
                                            ...
                                            <div className={clsx("flex flex-col gap-1", isAgent ? "items-end" : "items-start")}>
                                                <span className="text-xs font-bold text-gray-500 px-1">
                                                    {isAgent ? 'Support Agent' : selectedTicket.customerId?.name} • {moment(msg.timestamp).format('MMM DD, HH:mm')}
                                                </span>
                                                <div className={clsx(
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {selectedTicket.orderId && (
                                            <button className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 border border-indigo-100 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5">
                                                <ShieldAlert className="w-3.5 h-3.5" /> RMA Options
                                            </button>
                                        )}
                                    </div>
                                    <button
                                        ...
                                        className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:hover:bg-indigo-600 text-white px-6 py-2 rounded-xl font-bold shadow-md transition-all active:scale-95"
                                    >
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-gray-400 bg-gray-50/30">
                        ...
                        <p className="font-medium">Select a ticket from the inbox to start replying.</p>
                    </div>
                )}
            </div>
        </div>
