...
export default function Register() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (password.length < 12) {
            setError(t('common.passwordMinLength', 'Password must be at least 12 characters'));
            return;
        }
            ...
            <div className="relative z-10 flex-1 flex items-center justify-center px-4 py-8">
                <div className="w-full max-w-md">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className="text-indigo-200/60 text-sm mt-1">{t('registerTitle')}</p>
                    </div>
                    ...
                    <div className="backdrop-blur-xl bg-white/[0.07] border border-white/[0.1] rounded-2xl p-8 shadow-2xl shadow-black/20">
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            ...
                            {/* Full Name */}
                            <div>
                                <label htmlFor="reg-name" className={clsx("block text-sm font-medium text-indigo-100/80 mb-1.5", isAr ? "text-right" : "text-left")}>
                                    {t('fullName')}
                                </label>
                            ...
                            {/* Business Name */}
                            <div>
                                <label htmlFor="reg-business" className={clsx("block text-sm font-medium text-indigo-100/80 mb-1.5", isAr ? "text-right" : "text-left")}>
                                    {t('businessName')}
                                </label>
                                <div className="relative group">
                                    <div className={clsx("absolute inset-y-0 flex items-center pointer-events-none z-10 text-indigo-300/50 group-focus-within:text-indigo-400 transition-colors", isAr ? "right-0 pr-3.5" : "left-0 pl-3.5")}>
                            ...
                            {/* Email */}
                            <div>
                                <label htmlFor="reg-email" className={clsx("block text-sm font-medium text-indigo-100/80 mb-1.5", isAr ? "text-right" : "text-left")}>
                                    {t('emailAddress')}
                                </label>
                                <div className="relative group">
                                    <div className={clsx("absolute inset-y-0 flex items-center pointer-events-none z-10 text-indigo-300/50 group-focus-within:text-indigo-400 transition-colors", isAr ? "right-0 pr-3.5" : "left-0 pl-3.5")}>
                            ...
                            {/* Password */}
                            <div>
                                <label htmlFor="reg-password" className={clsx("block text-sm font-medium text-indigo-100/80 mb-1.5", isAr ? "text-right" : "text-left")}>
                                    {t('password')}
                                </label>
                                <div className="relative group">
                                    <div className={clsx("absolute inset-y-0 flex items-center pointer-events-none z-10 text-indigo-300/50 group-focus-within:text-indigo-400 transition-colors", isAr ? "right-0 pr-3.5" : "left-0 pl-3.5")}>
                                ...
                                <p className={clsx("text-xs text-white/25 mt-1.5", isAr ? "text-right" : "text-left")}>
                                    {t('common.passwordMinLength', 'Minimum 12 characters')}
                                </p>
                            </div>
                            ...
                            <button
                                ...
                                disabled={loading}
                                className={clsx(
                                    "group w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-semibold transition-all duration-200 mt-2",
