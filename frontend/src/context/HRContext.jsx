...
export const HRProvider = ({ children }) => {
    ...
    const recordPointage = async (employeeId, type, timestamp) => {
        try {
            setLoading(true);
            await api.post(`/api/hr/attendance/record`, { employeeId, type, timestamp });
    ...
    const generatePayroll = async (period) => {
        try {
            setLoading(true);
            await api.post(`/api/hr/payroll/generate`, { period });
    ...
    const approvePayroll = async (payrollId) => {
        try {
            setLoading(true);
            await api.put(`/api/hr/payroll/${payrollId}/approve`, {});
