...
export default function PageBuilder() {
  ...
  const [activePanel, setActivePanel] = useState('blocks'); // blocks | settings | preview
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] -mx-4 sm:-mx-8 lg:-mx-10 xl:-mx-14 2xl:-mx-16 -mt-10">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:flex items-center bg-gray-100 dark:bg-gray-700 rounded-lg p-0.5">
            <button onClick={() => setPreviewMode('desktop')} className={clsx('p-1.5 rounded-md transition-colors', previewMode === 'desktop' ? 'bg-white dark:bg-gray-600 shadow-sm' : 'text-gray-400')}>
              <Monitor className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setPreviewMode('mobile')} className={clsx('p-1.5 rounded-md transition-colors', previewMode === 'mobile' ? 'bg-white dark:bg-gray-600 shadow-sm' : 'text-gray-400')}>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 rounded-lg transition-colors"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex border-b border-gray-100 dark:border-gray-700">
            <button
              ...
              className={clsx('flex-1 py-2.5 text-xs font-semibold text-center transition-colors border-b-2',
                activePanel === 'blocks' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'
              )}
            >
              <Layers className="w-3.5 h-3.5 mx-auto mb-0.5" /> Blocks
            </button>
            <button
              onClick={() => setActivePanel('settings')}
              className={clsx('flex-1 py-2.5 text-xs font-semibold text-center transition-colors border-b-2',
                activePanel === 'settings' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'
              )}
            >
              <Settings className="w-3.5 h-3.5 mx-auto mb-0.5" /> Settings
            </button>
          </div>
            ...
            {activePanel === 'blocks' && (
              <div className="p-3 space-y-1">
                ...
                {blocks.map((block, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        'flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all text-xs group',
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 p-2.5 mt-2 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-blue-400 hover:text-blue-600 transition-colors text-xs font-medium"
                >
                ...
                {/* Block picker */}
                {showAddBlock && (
                  <div className="mt-2 bg-gray-50 dark:bg-gray-700/50 rounded-xl p-2 space-y-1 max-h-60 overflow-y-auto">
                    {BLOCK_TYPES.map(bt => (
                      <button
                        key={bt.type}
                        onClick={() => addBlock(bt.type)}
                        className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-white dark:hover:bg-gray-600 transition-colors text-xs"
                      >
                        <bt.icon className="w-3.5 h-3.5 text-gray-500" />
                        <span className="font-medium text-gray-700 dark:text-gray-300">{bt.label}</span>
                      </button>
                    ))}
                  </div>
                )}
        ...
        <div className="flex-1 bg-gray-100 dark:bg-gray-900 overflow-y-auto p-4 flex justify-center">
          <div className={clsx(
            'bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden transition-all duration-300',
        ...
        {selectedBlockData && (
          <div className="w-80 border-s border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-y-auto shrink-0">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700">
              ...
              <button onClick={() => setSelectedBlock(null)} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-4">
