            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                        <Box className="w-6 h-6 text-indigo-600" /> Enterprise Logistics
                    </h1>
                    <p className="text-gray-500 mt-1 font-medium">Manage Multi-Warehouse operations, Movement ledgers, and Supplier SLAs.</p>
                </div>
                <div className="flex gap-3 bg-gray-100 p-1.5 rounded-xl border border-gray-200">
                    ...
                    <button onClick={() => setActiveTab('ledger')} className={clsx("flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-all", activeTab === 'ledger' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-white/50')}>
                        <ArrowRightLeft className="w-4 h-4" /> Movement Ledger
                    </button>
                    <button onClick={() => setActiveTab('suppliers')} className={clsx("flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-all", activeTab === 'suppliers' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-white/50')}>
                        <ShieldCheck className="w-4 h-4" /> Supplier Metrics
                    </button>
                </div>
            </div>
                                    ...
                                    <div key={w._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col hover:border-indigo-200 hover:shadow-md transition-all">
                                        ...
                                        <div className="mt-2 space-y-2 mb-4">
                                            <div className="flex items-center gap-2 text-sm text-gray-600 font-medium">
                                                <MapPin className="w-4 h-4 text-gray-400" /> {w.location || 'No Location Set'}
                                            </div>
                                            <div className="flex items-center gap-2 text-sm text-gray-600 font-medium">
                                                <List className="w-4 h-4 text-gray-400" /> Capacity: {w.capacity.toLocaleString()} Units
                                            </div>
                                        </div>
                                        <div className="mt-auto border-t border-gray-100 pt-4 flex justify-between items-center text-sm font-semibold text-gray-500">
                                            <span>Manager: {w.managerDetails?.name || 'Unassigned'}</span>
                                        </div>
                                    </div>
                                )) : <div className="col-span-3 text-center text-gray-500 p-8 border border-dashed rounded-xl">No warehouses defined. Add one to enable multi-location tracking.</div>}
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'ledger' && (
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                                <h2 className="text-lg font-bold text-gray-900">Immutable Stock Ledger</h2>
                                <span className="text-xs font-bold bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full">Recent {ledger.length} entries</span>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-gray-100 text-sm">
                                        {ledger.map(entry => (
                                            <tr key={entry._id} className="hover:bg-gray-50/50">
                                                ...
                                                <td className="p-4 font-semibold text-gray-900">{entry.variantId?.sku || 'Deleted Variant'}</td>
                                                ...
                                                <td className="p-4 text-gray-600 font-medium">{entry.warehouseId?.name || 'Virtual / Unassigned'}</td>
                                                <td className="p-4 text-gray-500 text-xs">{entry.notes || '-'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                {ledger.length === 0 && <div className="p-8 text-center text-gray-500">No movement data found.</div>}
                            </div>
                        </div>
                    )}
                                ...
                                <div key={s._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col hover:border-gray-300">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-gray-500 border-b border-gray-100 pb-2 inline-block">Code: {s.code}</p>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                                            <div className="text-xs text-gray-500 font-bold mb-1">Lead Time</div>
                                            <div className="text-xl font-bold text-gray-900">{s.averageLeadTimeDays} <span className="text-xs text-gray-500">Days</span></div>
                                        </div>
                                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                                            <div className="text-xs text-gray-500 font-bold mb-1">Reliability</div>
                                    ...
                                    <div className="text-sm text-gray-600 font-medium flex gap-2 items-center">
                                        <AlertTriangle className="w-4 h-4 text-orange-400" /> Defect Rate: {s.defectRatePercentage}%
                                    </div>
                                </div>
                            ))}
                            {suppliers.length === 0 && <div className="col-span-3 p-8 text-center text-gray-500 border border-dashed rounded-xl">No suppliers trackable. Use the main Inventory tab to add Vendors.</div>}
                        </div>
                    )}
                </>
            )}
                ...
                <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                        <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between">
                            <h2 className="text-lg font-bold">Register Warehouse</h2>
                        </div>
                        <form onSubmit={handleCreateWarehouse} className="p-6 flex flex-col gap-4">
                            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Warehouse Name</label><input type="text" required value={warehouseForm.name} onChange={e => setWarehouseForm({ ...warehouseForm, name: e.target.value })} className="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-100 outline-none" /></div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Code</label><input type="text" required value={warehouseForm.code} onChange={e => setWarehouseForm({ ...warehouseForm, code: e.target.value })} className="w-full border rounded-lg p-2 uppercase focus:ring-2 focus:ring-indigo-100 outline-none" /></div>
                                <div><label className="block text-sm font-semibold text-gray-700 mb-1">Capacity</label><input type="number" required value={warehouseForm.capacity} onChange={e => setWarehouseForm({ ...warehouseForm, capacity: Number(e.target.value) })} className="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-100 outline-none" /></div>
                            </div>
                            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Location / Address</label><input type="text" value={warehouseForm.location} onChange={e => setWarehouseForm({ ...warehouseForm, location: e.target.value })} className="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-100 outline-none" /></div>
                            <div className="flex justify-end gap-3 mt-4">
                                <button type="button" onClick={() => setIsWarehouseModalOpen(false)} className="px-5 py-2 font-bold text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
                                <button type="submit" className="px-5 py-2 font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Save Warehouse</button>
                            </div>
                        </form>
                    </div>
                </div>
