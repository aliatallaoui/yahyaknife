...
export default function EmployeeProfile() {
    ...
    if (!employee) return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-50 text-gray-500">
            ...
            <button onClick={() => navigate('/hr')} className="mt-4 text-blue-600 hover:underline">Return to Directory</button>
        </div>
    );
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 flex flex-col items-center relative overflow-hidden">
                    ...
                    <div className="w-full space-y-4">
                        <ProfileRow icon={HashIcon} label="Employee ID" value={employee.employeeId || employee._id.slice(-6).toUpperCase()} />
                        <ProfileRow icon={LayersIcon} label="Department" value={employee.department} />
                        <ProfileRow icon={Calendar} label="Start Date" value={moment(employee.joinDate).format('MMMM Do, YYYY')} />
                        <ProfileRow icon={Mail} label="Email" value={employee.email || 'N/A'} />
                        <ProfileRow icon={Activity} label="Contract Status" value={employee.status} highlight={employee.status === 'Active'} />
                    </div>
                </div>
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 lg:p-8 flex flex-col justify-between">
                    <div>
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"><Wallet className="w-5 h-5 text-blue-500" /> Live Payroll Projection</h3>
                            <span className="text-xs font-bold bg-blue-50 text-blue-600 px-3 py-1 rounded-full">{moment().format('MMMM YYYY')}</span>
                        </div>

                        <div className="mb-8">
                            <p className="text-sm font-semibold text-gray-500 mb-1">Estimated Net Payout</p>
                            <div className="flex items-end gap-2">
                                <span className="text-5xl font-black text-gray-900 tracking-tight">{estGrossSalary.toLocaleString()}</span>
                                <span className="text-lg font-bold text-gray-400 mb-1">DZD</span>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <FinanceRow label="Monthly Base Salary" value={baseSalary} type="base" />
                            <FinanceRow label="Overtime Bonus" value={overtimeBonus} type="addition" />
                            <FinanceRow label="Lateness & Missing" value={missingDeductions} type="deduction" />
                            <FinanceRow label="Absence Penalty" value={absenceDeduct} type="deduction" />
                        </div>
                    </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100">
                        <div className="flex justify-between items-center bg-gray-50 rounded-2xl p-4">
                            <div>
                                <p className="text-xs font-bold text-gray-400 mb-0.5">ALREADY PAID THIS MONTH</p>
                                <p className="text-lg font-bold text-emerald-600">{paidThisMonth.toLocaleString()} DZD</p>
                            </div>
                            <div className="text-right">
                                <p className="text-xs font-bold text-gray-400 mb-0.5">REMAINING TO PAY</p>
                                <p className="text-lg font-bold text-gray-900">{unpaidEst.toLocaleString()} DZD</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex-1">
                        <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-5">Today's Pulse</h3>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <TimeBlock label="Morning In" time={formatHHMM(todayAtt?.morningIn)} />
                            <TimeBlock label="Morning Out" time={formatHHMM(todayAtt?.morningOut)} />
                            <TimeBlock label="Evening In" time={formatHHMM(todayAtt?.eveningIn)} />
                            <TimeBlock label="Evening Out" time={formatHHMM(todayAtt?.eveningOut)} />
                        </div>
                    </div>

                    {/* Quick Stats Grid */}
                    <div className="grid grid-cols-2 gap-4">
                        <StatSquare label="Worked Days" value={workedDaysAmount} icon={Calendar} color="bg-blue-50 text-blue-600" />
                        <StatSquare label="Absences" value={absencesAmount} icon={XCircle} color="bg-rose-50 text-rose-600" />
                        <StatSquare label="Lates" value={latesAmount} icon={Clock} color="bg-amber-50 text-amber-600" />
                        <StatSquare label="Overtime" value={formatHours(totalOvertimeMin)} icon={TrendingUp} color="bg-purple-50 text-purple-600" />
                    </div>
                </div>
            </div>
                ...
                <div className="border-b border-gray-100 bg-gray-50/50 p-6 flex flex-col xl:flex-row items-center justify-between gap-6">
                    ...
                    <div className="flex-1 w-full max-w-xl">
                        <div className="flex justify-between items-end mb-2">
                            <div>
                                <h4 className="text-sm font-bold text-gray-900">Monthly Time Fulfillment</h4>
                                <p className="text-xs text-gray-500">Cumulative worked hours vs quota</p>
                            </div>
                            <div className="text-right">
                                <span className="text-lg font-black text-blue-600">{Math.floor(totalWorkedMin / 60)}h</span>
                                <span className="text-sm font-bold text-gray-400"> / {Math.floor(totalRequiredMin / 60)}h</span>
                            </div>
                        </div>
                        ...
                        <div className="mt-2 text-xs font-bold text-rose-500 text-right">
                            {totalMissingMin > 0 && `${formatHours(totalMissingMin)} missing globally`}
                        </div>
                    </div>

                    {/* Tabs */}
                    <div className="flex bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm shrink-0">
                        <TabButton active={activeTab === 'attendance'} onClick={() => setActiveTab('attendance')} icon={Clock} label="Timesheets" />
                        <TabButton active={activeTab === 'payroll'} onClick={() => setActiveTab('payroll')} icon={FileText} label="Payroll History" />
                        <TabButton active={activeTab === 'leaves'} onClick={() => setActiveTab('leaves')} icon={Calendar} label="Leave History" />
                    </div>
                </div>
                            ...
                            <table className="w-full text-left border-collapse min-w-[900px]">
                                <thead>
                                    <tr className="bg-white border-b border-gray-100 text-gray-400 text-xs uppercase tracking-wider font-bold">
                                        ...
                                        <th className="p-4">Late / Missing</th>
                                ...
                                <tbody className="divide-y divide-gray-50 text-sm font-medium">
                                    {attendance.slice(0, 31).map(att => (
                                        <tr key={att._id} className="hover:bg-gray-50/50 transition-colors">
                                            <td className="p-4 pl-6 text-gray-900 font-bold">{moment(att.date).format('MMM DD, YYYY')}</td>
                                            ...
                                            <td className="p-4">
                                                <div className="flex flex-col gap-1 text-xs">
                                                    {att.lateMinutes > 0 && <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded w-fit">{att.lateMinutes}m late</span>}
                                                    {att.missingMinutes > 0 && <span className="text-rose-600 bg-rose-50 px-2 py-0.5 rounded w-fit">{formatHours(att.missingMinutes)} miss</span>}
                                                    {!att.lateMinutes && !att.missingMinutes && <span className="text-gray-300">-</span>}
                                                </div>
                                            </td>
                                            <td className="p-4 text-purple-600 font-bold">{att.overtimeMinutes > 0 ? `+${formatHours(att.overtimeMinutes)}` : '-'}</td>
                                    ...
                                    {attendance.length === 0 && (
                                        <tr><td colSpan="7" className="p-10 text-center text-gray-500 font-medium">No attendance records found for this employee.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                            ...
                            <table className="w-full text-left border-collapse min-w-[800px]">
                                <thead>
                                    <tr className="bg-white border-b border-gray-100 text-gray-400 text-xs uppercase tracking-wider font-bold">
                                        ...
                                        <th className="p-4">Overtime Additions</th>
                                ...
                                <tbody className="divide-y divide-gray-50 text-sm font-medium">
                                    {payrolls.map(pay => (
                                        <tr key={pay._id} className="hover:bg-gray-50/50 transition-colors">
                                            <td className="p-4 pl-6 text-gray-900 font-bold">{moment(pay.period, 'MM-YYYY').format('MMMM YYYY')}</td>
                                            <td className="p-4 text-gray-500">{pay.baseSalary?.toLocaleString()} DZD</td>
                                            <td className="p-4 text-emerald-600">+{(pay.overtimeAdditions || 0).toLocaleString()} DZD</td>
                                            <td className="p-4 text-rose-600">-{(pay.missingTimeDeductions + pay.absenceDeductions || 0).toLocaleString()} DZD</td>
                                            <td className="p-4 font-black text-blue-600 text-lg">{pay.finalPayableSalary?.toLocaleString()} DZD</td>
                                            <td className="p-4 pr-6 text-right">
                                    ...
                                    {payrolls.length === 0 && (
                                        <tr><td colSpan="6" className="p-10 text-center text-gray-500 font-medium">No payroll history found.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    ...
                    {activeTab === 'leaves' && (
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse min-w-[800px]">
                                <thead>
                                    <tr className="bg-white border-b border-gray-100 text-gray-400 text-xs uppercase tracking-wider font-bold">
                                        <th className="p-4 pl-6">Requested On</th>
                                ...
                                <tbody className="divide-y divide-gray-50 text-sm font-medium">
                                    {leaves.map(l => (
                                        <tr key={l._id} className="hover:bg-gray-50/50 transition-colors">
                                            <td className="p-4 pl-6 text-gray-900 font-bold">{moment(l.requestDate).format('MMM DD, YYYY')}</td>
                                            <td className="p-4 text-gray-700">
                                                <span className="bg-gray-100 px-2.5 py-1 rounded-md text-xs font-bold">{l.type}</span>
                                            </td>
                                            <td className="p-4 text-gray-500">
                                                {moment(l.startDate).format('MMM DD')} ➔ {moment(l.endDate).format('MMM DD')} <span className="text-gray-400 font-normal ml-2">({moment(l.endDate).diff(moment(l.startDate), 'days') + 1} days)</span>
                                            </td>
                                    ...
                                    {leaves.length === 0 && (
                                        <tr><td colSpan="5" className="p-10 text-center text-gray-500 font-medium">No leave requests found.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
    ...
    return (
        <div className="flex justify-between items-center text-sm">
            ...
            <span className={clsx("font-bold tabular-nums",
                ...
                {type === 'base' ? '' : type === 'addition' ? '+' : '-'} {value.toLocaleString()} <span className="text-xs text-gray-400 font-normal">DZD</span>
            </span>
        </div>
    );
}
