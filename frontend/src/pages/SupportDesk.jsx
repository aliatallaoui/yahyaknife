...
export default function SupportDesk() {
    ...
    const getPriorityColor = (p) => {
        switch (p) {
            case 'Urgent': return 'text-rose-600 bg-rose-50 border-rose-200 dark:text-rose-400 dark:bg-rose-900/20 dark:border-rose-800';
            ...
            default: return 'text-emerald-600 bg-emerald-50 border-emerald-200 dark:text-emerald-400 dark:bg-emerald-900/20 dark:border-emerald-800';
        }
    };
    ...
    return (
        <div className="flex flex-col gap-4 h-[calc(100vh-6rem)] sm:h-[calc(100vh-6.5rem)]">
            ...
            {loading ? (
                <div className="flex-1 flex items-center justify-center bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
                    <div className="flex flex-col items-center gap-3">
                        <div className="w-8 h-8 border-[3px] border-emerald-200 dark:border-emerald-800 border-t-emerald-500 rounded-full animate-spin" />
                        <span className="text-sm text-gray-400 dark:text-gray-500 font-medium">{t('common.loading', 'Loading...')}</span>
                    </div>
                </div>
            ) : (
            <div className="flex flex-col md:flex-row flex-1 bg-white dark:bg-gray-800 overflow-hidden font-sans border border-gray-100 dark:border-gray-700 rounded-3xl shadow-sm">
                {/* Sidebar List */}
                <div className={clsx(
                    "w-full md:w-[340px] lg:w-[400px] bg-white dark:bg-gray-800 border-b md:border-b-0 md:border-e border-gray-100 dark:border-gray-700 flex flex-col z-10 relative shrink-0",
                    selectedTicket ? "hidden md:flex md:h-full" : "h-full md:h-full"
                )}>
                    <div className="p-5 border-b border-gray-100 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 backdrop-blur-md sticky top-0">
                        <div className="relative mb-4">
                            ...
                            <input
                                ...
                                className="w-full ps-9 pe-4 py-2 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow outline-none dark:bg-gray-700 dark:text-gray-100 dark:placeholder-gray-500"
                            />
                        </div>
                                ...
                                >
                                    ...
                                    {f.count > 0 && (
                                        <span className={clsx(
                                            ...
                                            filter === f.value ? "bg-white/30 text-white" : "bg-white dark:bg-gray-600 text-gray-600 dark:text-gray-300"
                                        )}>{f.count}</span>
                                    )}
                                </button>
                            ))}
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700 p-2">
                        {loading ? (
                            <div className="p-8 text-center text-gray-400 dark:text-gray-500">{t('crm.loadingTickets', 'Loading tickets...')}</div>
                        ) : filteredTickets.length === 0 ? (
                            <div className="p-8 text-center text-gray-400 dark:text-gray-500 flex flex-col items-center">
                                <CheckCircle2 className="w-10 h-10 text-emerald-300 dark:text-emerald-600 mb-2" />
                                ...
                                <button
                                    ...
                                    className={clsx(
                                        ...
                                        selectedTicket?._id === ticket._id
                                            ? "bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 shadow-sm"
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-bold text-gray-900 dark:text-white truncate pe-2">{ticket.customerId?.name || 'Unknown'}</span>
                                        </div>
                                        <span className="text-xs font-medium text-gray-400 dark:text-gray-500 whitespace-nowrap">
                                            {fromNowShort(ticket.createdAt)}
                                        </span>
                                    </div>
                                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 truncate pe-8 mb-2">
                                        {ticket.subject}
                                    </p>
                ...
                {/* Conversation View */}
                <div className={clsx(
                    "flex-1 flex flex-col bg-white dark:bg-gray-800 h-full relative",
                    !selectedTicket && "hidden md:flex"
                )}>
                    {selectedTicket ? (
                        <>
                            {/* Header Details */}
                            <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col md:flex-row justify-between items-start shadow-sm z-10 shrink-0 gap-3 sm:gap-4">
                                <div className="min-w-0 w-full">
                                    <div className="flex items-start gap-2 mb-2">
                                        <button onClick={() => setSelectedTicket(null)} className="md:hidden p-1 -ms-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 shrink-0 mt-1">
                                        ...
                                        <h2 className="text-lg sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate">{selectedTicket.subject}</h2>
                                    ...
                                    <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 font-medium">
                                        ...
                                        <span>•</span>
                                        <span className="text-gray-900 dark:text-white font-bold">{selectedTicket.customerId?.name}</span>
                                        <span>•</span>
                                        <span>{selectedTicket.type}</span>
                                    </div>
                                </div>

                                <div className="flex flex-wrap items-center gap-2 bg-gray-50 dark:bg-gray-700/50 p-1.5 rounded-xl border border-gray-200 dark:border-gray-600">
                                        ...
                                        if (isSystem) {
                                            return (
                                                <div key={idx} className="flex justify-center my-4">
                                                    <span className="text-xs font-bold text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full border border-gray-200 dark:border-gray-600">
                                                        {msg.message}
                                                    </span>
                                                </div>
                                            );
                                            ...
                                            <div key={idx} className={clsx("flex gap-4 max-w-[85%]", isAgent ? "mis-auto flex-row-reverse" : "")}>
                                                <div className="shrink-0">
                                                    <div className={clsx(
                                                        ...
                                                        isAgent ? "bg-indigo-600 text-white border-indigo-700" : "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800"
                                                    )}>
                                                ...
                                                <div className={clsx("flex flex-col gap-1", isAgent ? "items-end" : "items-start")}>
                                                    ...
                                                    <div className={clsx(
                                                        ...
                                                        isAgent
                                                            ...
                                                            : "bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 border border-gray-100 dark:border-gray-600 rounded-tl-sm rtl:rounded-tl-2xl rtl:rounded-tr-sm"
                                                    )}>
                                                        {msg.message}
                                                    </div>
                                                </div>
                            ...
                            {/* Composer Bottom */}
                            <div className="p-4 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-600 shrink-0">
                                {replyError && (
                                    <div className="mb-3 flex items-center gap-2 text-sm text-rose-700 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-lg px-3 py-2">
                                        <AlertCircle className="w-4 h-4 shrink-0" />
                                ...
                                <div className="max-w-3xl mx-auto border border-gray-200 dark:border-gray-600 rounded-2xl shadow-sm focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition-all bg-white dark:bg-gray-700 relative overflow-hidden flex flex-col">
                                    <textarea
                                        className="w-full p-4 text-sm resize-none outline-none bg-transparent min-h-[120px] dark:text-gray-100 dark:placeholder-gray-500"
                                        placeholder={t('crm.replyPlaceholder', "Type your reply to the customer...")}
                                        value={replyText}
                                        onChange={(e) => setReplyText(e.target.value)}
                                        autoFocus
                                    />
                                    <div className="bg-gray-50 dark:bg-gray-800 px-4 py-3 border-t border-gray-100 dark:border-gray-600 flex justify-between items-center mt-auto flex-wrap gap-2">
                                        <div className="flex gap-2">
                                            <button disabled={!hasPermission('support.edit')} className="text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 px-3 py-1.5 rounded-lg shadow-sm transition-colors disabled:opacity-50">
                                                {t('crm.btnInsertTemplate', 'Insert Template')}
                                            </button>
                                            {selectedTicket.orderId && hasPermission('support.edit') && (
                                                <button className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 shadow-sm">
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 bg-gray-50/30 dark:bg-gray-900/30">
