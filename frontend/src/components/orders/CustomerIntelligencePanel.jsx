    ...
    if (isSearching) {
        return (
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 flex items-center justify-center min-h-[100px] animate-pulse">
                <p className="text-sm text-gray-500 font-medium tracking-wide">{t('customerIntel.searching')}</p>
            </div>
        );
    }
        ...
        <div className="bg-white border shadow-sm rounded-xl p-4 space-y-4">
            ...
            {exists && (
                <div className="grid grid-cols-4 gap-4 pt-2">
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">{t('customerIntel.orders')}</span>
                        <span className="text-lg font-bold text-gray-900">{customer.totalOrders}</span>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">{t('customerIntel.delivered')}</span>
                        <span className="text-lg font-bold text-gray-900">{customer.deliveredOrders}</span>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider">{t('customerIntel.returnRate')}</span>
            ...
            {activeDuplicateOrders && activeDuplicateOrders.length > 0 && (
                <div className="mt-4 pt-3 border-t">
                    <h4 className="text-xs font-bold text-gray-900 mb-2 uppercase tracking-wide">{t('customerIntel.duplicates')}</h4>
