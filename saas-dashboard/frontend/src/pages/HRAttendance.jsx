    ...
    return (
        <div className="p-8 relative">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold border-b-4 border-indigo-600 pb-2 inline-block">{t('hr.attendanceTitle')}</h1>
                    <p className="text-gray-500 mt-2">{t('hr.attendanceSubtitle')}</p>
                </div>
                <div className="flex gap-4">
                    <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
                        className="border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 font-medium text-gray-700 px-3 py-2 border outline-none cursor-pointer ltr:text-left rtl:text-right" />
                    <button onClick={fetchAttendance} disabled={loading} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold shadow-md hover:bg-indigo-700 disabled:bg-indigo-300">
                        <RefreshCcw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} /> {loading ? t('hr.syncing') : t('hr.btnLiveSync')}
                    </button>
                </div>
            </div>
