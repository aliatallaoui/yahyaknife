        ...
        <div className="w-full space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {loadingCouriers ? (
                    ...
                    <div className="col-span-full py-12 text-center text-gray-500 font-medium border-2 border-dashed border-gray-200 rounded-2xl">{t('general.no_data')}</div>
                ) : (
                    couriers.map(courier => (
                        <div key={courier._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col justify-between group">
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center font-bold text-gray-700">
                                        <Truck className="w-5 h-5 text-gray-400" />
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-gray-900 leading-tight">{courier.name}</h3>
                                        <p className="text-xs text-gray-500 font-medium">{courier.integrationType}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="p-3 bg-rose-50 rounded-xl border border-rose-100/50">
                                    <p className="text-[10px] uppercase font-bold text-rose-500 mb-1">{t('finance.pendingRemittance', 'Pending Cash Drop')}</p>
                                    <p className="text-xl font-black text-rose-700">{courier.pendingRemittance?.toLocaleString() || 0} DZD</p>
                                </div>

                                <button
                                    onClick={() => handleSelectCourier(courier)}
                                    disabled={courier.pendingRemittance <= 0}
                                    className="w-full py-2.5 bg-gray-900 hover:bg-black text-white rounded-xl text-sm font-bold transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-sm"
                                >
            ...
            {selectedCourier && (
                <div className="fixed inset-0 z-50 flex justify-end bg-gray-900/60 backdrop-blur-sm animate-in fade-in">
                    <div className="w-full sm:max-w-3xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right-8 duration-300">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-white space-y-4">
                            {error && (
                                <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-sm font-bold flex gap-2 items-center">
                            ...
                            {loadingDeliveries ? (
                                ...
                                <div className="py-20 text-center text-gray-500 font-medium bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                                ...
                                                return (
                                                    <tr key={d._id} className={clsx("cursor-pointer", isSelected && "row-selected")} onClick={() => toggleOrderSelection(d._id)}>
                                                        <td className="px-4 py-3">
                                                            <div className={clsx("w-5 h-5 rounded border flex items-center justify-center transition-colors", isSelected ? "bg-emerald-500 border-emerald-500" : "border-gray-300")}>
                                                                {isSelected && <CheckCircle className="w-3.5 h-3.5 text-white" />}
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-3 font-mono text-xs font-bold text-gray-900">{d.orderId || d._id.slice(-6)}</td>
                                                        <td className="px-4 py-3">
                        ...
                        <div className="p-6 bg-gray-50 border-t border-gray-200">
                            <div className="mb-4 space-y-3">
                                <div className="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                                    <span className="text-sm font-bold text-gray-500 uppercase tracking-wide">{t('finance.expectedFromSelected', 'Expected from Selected')} ({selectedOrderIds.size})</span>
                                    <span className="text-2xl font-black text-gray-900">{calculatedTotal.toLocaleString()} {t('common.dzd', 'DZD')}</span>
                                </div>
                                
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="text-xs font-bold text-gray-500 uppercase pb-1 block">{t('finance.actualCashHandedOver', 'Actual Cash Handed Over')}</label>
                                        <input 
                                            type="number" 
                                            placeholder={calculatedTotal || '0'}
                                            value={customAmount}
                                            onChange={(e) => setCustomAmount(e.target.value)}
                                            className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl font-black text-lg text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-500 transition-all placeholder:text-gray-300"
                                        />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-gray-500 uppercase pb-1 block">{t('finance.notesOptional', 'Notes (Optional)')}</label>
                                        <input 
                                            type="text" 
                                            placeholder={t('finance.settlementNotesPlaceholder', 'e.g. Missing 500 DZD for gas')}
                                            value={settlementNotes}
                                            onChange={(e) => setSettlementNotes(e.target.value)}
                                            className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl font-medium outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-300 text-white rounded-xl font-bold text-lg flex justify-center items-center gap-2 transition-all active:scale-[0.98] shadow-md"
                            >
