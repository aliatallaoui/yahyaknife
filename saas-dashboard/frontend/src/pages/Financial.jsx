            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-[500px]">
                    ...
                    <table className="w-full text-start border-collapse min-w-[600px]">
                        <thead>
                            <tr className="bg-gray-50/80 text-gray-500 text-xs uppercase tracking-wider">
                                ...
                                <th className="p-4 font-semibold">{t('finance.type', 'Type')}</th>
                                <th className="p-4 font-semibold">{t('finance.category', 'Category')}</th>
                                <th className="p-4 font-semibold">{t('finance.desc', 'Description')}</th>
                                <th className="p-4 font-semibold text-end">{t('finance.amount', 'Amount')}</th>
                                <th className="p-4 font-semibold text-center w-24">{t('finance.actions', 'Actions')}</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={tx._id} className="hover:bg-gray-50/50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        <span className={clsx("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold",
                                            ...
                                            {t(`finance.${tx.type}`, tx.type.charAt(0).toUpperCase() + tx.type.slice(1))}
                                        </span>
                                    </td>
                                    ...
                                    <td className={clsx("p-4 text-end font-bold tabular-nums",
                                        tx.type === 'revenue' ? "text-emerald-700" : "text-gray-900"
                                    )}>
                                        {tx.type === 'revenue' ? '+' : '-'}{tx.amount.toLocaleString()} {t('common.currency', 'DZ')}
                                    </td>
                                    <td className="p-4">
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between p-4 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
                        <span className="text-sm font-medium text-gray-500">{t('finance.page')} {currentPage} {t('finance.of')} {totalPages}</span>
